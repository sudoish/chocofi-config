...
0=BASE, 1=BASE_WIN, 2=NUM, 3=NAV, 4=NAVW, 5=SYM, 6=SYMW, 7=ADJ, 8=LOCK

BASE and BASE_WIN are OS-specific base layers (macOS/Windows). NAV/NAVW and SYM/SYMW are similarly duplicated. Conditional layers activate NAVW/SYMW when BASE_WIN is active, and ADJ is a tri-layer (NAV+SYM or NAVW+SYMW).

### Key Config Files

- `config/combos.dtsi` — combo definitions (uses macro `COMBO(name, positions, binding, layers)`)
- `config/macros.dtsi` — ZMK macros (screen switching, undo/redo)
- `config/mouse.dtsi` — mouse emulation settings
- `config/extra_keys.h` — preprocessor defines for key layout variants (`ALPHA_13332` vs `ALPHA_23332` vs full 3x5)
...
- **Tri-state swapper**: Alt-Tab behavior using `zmk,behavior-tri-state` (requires Urob's fork)
- **OS-aware layers**: BASE_WIN overlays only the keys that differ (CTRL↔GUI swap for HRMs), using `&trans` for shared keys
- **Conditional layers**: Tri-layer pattern for ADJ and automatic OS-variant activation
